    ...
    def __init__(
        ...
        for vol_id, file in enumerate(files):
            ...
            for coil in range(volume_k.shape[1]):
                # Normalize
                eq_modulus[:,coil,...] = normalize_hist(modulus[0,coil,...])
                eq_argument[:,coil,...] = normalize_hist(argument[0,coil,...])
                
                # Equalize
                eq_modulus[:,coil,...] = exposure.equalize_adapthist(eq_modulus[0,coil,...],  clip_limit=0.15)
                eq_argument[:,coil,...] = exposure.equalize_adapthist(eq_argument[0,coil,...],  clip_limit=0.15)

                # Denormalize back
                eq_modulus[:,coil,...] = rescale_hist(eq_modulus[0,coil,...], np.min(modulus[0,coil,...]), np.max(modulus[0,coil,...]))
                eq_argument[:,coil,...] = rescale_hist(eq_argument[0,coil,...], np.min(argument[0,coil,...]), np.max(argument[0,coil,...]))

            # Rebuild the new equalized version of the kspace values
            ...
            eq_rebuilt_kspace = torch.complex(eq_re, eq_im)
            
            # Fill up the volume with the new equalized kspace values
            volume_kspace[...,-2] = torch.real(eq_rebuilt_kspace)
            volume_kspace[...,-1] = torch.imag(eq_rebuilt_kspace)
